...
class RubikCube:
    ...
    def apply_move(self, move):
        # Aplica un movimiento en notación (por ejemplo, "U", "U'", etc.)
        if move.endswith("'"):
            clockwise = False
            move = move[0]
        else:
    ...
    def rotate_cube_y(self, clockwise=True):
        ...
        Se usa para generar estados simétricos.
        """
        new_cube = RubikCube()
        ...
        if clockwise:
            # La rotación sobre Y reordena las caras laterales:
            # Nueva F = antigua L (rotada 90° en sentido horario)
            ...
            # Para sentido antihorario, se pueden usar 3 rotaciones horarias (90° * 3 = 270°) equivalentes a 90° CCW.
    ...
    def canonical_state(self):
        """
        Genera la representación canónica del estado del cubo, considerando
        las 4 rotaciones posibles sobre el eje vertical (Y). Se retorna el mínimo
        (lexicográficamente) de las representaciones.
        """
        states = []
        current = self.clone()
        for _ in range(4):
...
class Solver:
    def __init__(self):
        # Caché para heurística (usando la representación canónica del estado)
    ...
    def get_heuristic(self, cube):
        # Usar la representación canónica para la caché.
    ...
    def search(self, cube, g, threshold, path, previous_move):
        ...
        if cube.is_solved():
            return 'FOUND'
        min_cost = float('inf')
        # Generar sucesores y priorizarlos según la heurística.
        successors = []
        for move in self.moves:
            # Evitar movimientos redundantes (por ejemplo, no invertir inmediatamente un movimiento)
