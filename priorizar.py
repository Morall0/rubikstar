...
class RubikCube:
    ...
    # Cada cara se representa como una lista de 9 elementos (índices 0..8):
    # 0 1 2
    # 3 4 5
    # 6 7 8
    CORNER_INDICES = [
...
class Solver:
    ...
    def search(self, cube, g, threshold, path):
        ...
        min_cost = float('inf')
        successors = []
        # Generar sucesores y almacenar su heurística
        for move in self.moves:
            new_cube = cube.clone()
            new_cube.apply_move(move)
            h = self.get_heuristic(new_cube)
            successors.append((h, move, new_cube))
        # Priorizar movimientos según la heurística (menor h primero)
