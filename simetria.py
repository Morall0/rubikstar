...
class RubikCube:
    ...
    # Posiciones de las esquinas (cada una con 3 stickers)
    # Cada cara se representa como lista de 9 elementos (índices 0..8):
    # 0 1 2
    # 3 4 5
    # 6 7 8
    CORNER_INDICES = [
    ...
    # Posiciones de las aristas (cada una con 2 stickers)
    EDGE_INDICES = [
        ((0,7), (4,1)),   # UF
        ((0,5), (3,1)),   # UR
        ((0,1), (5,1)),   # UB
    ...
    def move_L(self, clockwise=True):
        ...
        if clockwise:
            ...
            temp = [self.cube[0][0], self.cube[0][3], self.cube[0][6]]
            self.cube[0][0], self.cube[0][3], self.cube[0][6] = self.cube[4][:3]
            self.cube[4][:3] = self.cube[1][:3]
            self.cube[1][:3] = self.cube[5][:3]
            self.cube[5][:3] = temp
    ...
    def rotate_cube_y(self, clockwise=True):
        """
        Rota el cubo completo 90° sobre el eje vertical (Y).
        Esto reordena las caras laterales de forma que se pueda considerar la simetría.
        """
        new_cube = RubikCube()
        # Las caras U y D permanecen iguales.
        new_cube.cube[0] = self.cube[0][:]
        new_cube.cube[1] = self.cube[1][:]
        if clockwise:
            # Nueva F = antigua L (rotada 90°)
    ...
    def canonical_state(self):
        """
        Genera la representación canónica del estado del cubo,
        considerando las 4 rotaciones posibles sobre el eje vertical (Y).
        Se retorna el mínimo (lexicográficamente) de las representaciones.
        """
        states = []
        current = self.clone()
        for _ in range(4):
    ...
    def canonical_cube(self):
        """
        Retorna una nueva instancia de RubikCube cuyo estado es el canónico
        (es decir, el que representa la simetría mínima) del cubo actual.
        """
        canon = self.canonical_state()
        new_cube = RubikCube()
        new_cube.cube = [ list(face) for face in canon ]
        return new_cube
...
class Solver:
    ...
    def get_heuristic(self, cube):
        # Se aplica simetría: se evalúa la heurística en el estado canónico.
        canonical_cube = cube.canonical_cube()
        return canonical_cube.heuristic()
